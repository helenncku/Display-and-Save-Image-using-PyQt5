...
class HelenApp(QMainWindow):
  ...
  # function show Image to LbImage
  ...
  def openImagefile(self):
    global filename
    filename, img = QFileDialog.getOpenFileName(self, 'open file') 
  ...
  def DeepLabV3plus(self):
    #pixmap = QPixmap(current_filename) 
    #print(current_filename)
    self.showImage2LbImage(self.ui.segmentation_box, filename)
    
  def saveImage(self):
    savefile, _ = QFileDialog.getSaveFileName(self, "Save Image", os.curdir, "JPG Files (*.jpg)")
    pixmap = QPixmap(filename)
...
#  def openImage(self):
#    self.showImage2LbImage(self.ui.imagebox, self.openfile)
      

    # this code given error: can not convert string to float
